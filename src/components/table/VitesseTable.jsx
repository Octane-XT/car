...
import VitesseForm from '../forms/vitesse/VitesseForm';

const VitesseTable = ({ data, onDelete }) => {
  ...
  const filteredData = data.filter((item) =>
    item.nomvitesse.toLowerCase().includes(filter.toLowerCase())
  );
    ...
    <div className="container mt-5">
      ...
      <div className="mb-3 mt-5">
        <input
          ...
          placeholder="Filtre par nom du vitesse"
          value={filter}
      ...
      <table className="table">
        ...
        <tbody>
          {filteredData.map((item, index) => (
            <tr key={index}>
              <td>{item.idvitesse}</td>
              <td>{item.nomvitesse}</td>
              <td>
                <button className="btn btn-primary mr-2" onClick={() => handleEdit(item)}>
                  Modifier
                </button>
                <button style={{ marginLeft: 15 }} className="btn btn-danger" onClick={() => handleDelete(item.idvitesse)}>
                  Supprimer
                </button>
              </td>
            </tr>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <VitesseForm rowdata={selectedRowData} onModalClose={handleCloseModal} />
              </div>
            </div>
          </div>
        </div>
